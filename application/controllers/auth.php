...
class Auth extends CI_Controller
{
	...
	public function create_user()
	{
		...
		if (!$this->ion_auth->logged_in() || !$this->ion_auth->is_admin()) {
//			redirect('auth'.DS.'create_user', 'refresh');
		}
		...
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($identity, $password, $email, $additional_data)) {
			...
			if ($email) {
				$info = $this->db->get_where('users', array('email' => $email))->result_array();
				$this->db->insert('tbl_student', array(
					'student_name' => $info['0']['last_name'],
					'student_phone' => $info['0']['phone'],
					'user_id' => $info['0']['id'], 'student_email' => $email
				));
			}
			$this->session->set_flashdata('message', $this->ion_auth->messages());
			redirect("auth", 'refresh');
